		...
		{:else}
			{#each labFiles as file (file)}
				<li class="row">
					...
					<div class="actions">
						<button onclick={() => handleCorrect(file)} title="Correct in ScanTailor"
							>Correct</button
						>
						<button onclick={() => handleFinalize(file)} title="Mark as done and move to OCR"
							>Finalized</button
						>
					</div>
				</li>
