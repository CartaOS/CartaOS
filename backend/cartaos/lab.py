...
class LabProcessor:
    ...
    def process(self) -> bool:
        ...
        try:
            ...
            if not corrected_images:
                logger.warning(
                    "No corrected images found in the ScanTailor output directory. The PDF will not be recreated."
                )
                return False
    ...
    def _run_unpaper_cleanup(self, workspace: str, images: List[Path]) -> None:
        ...
        for image in images:
            ...
            subprocess.run(
                ["unpaper", str(image), str(output_image)],
                cwd=workspace,
                check=True,
                capture_output=True,
            )
    ...
    def _create_scantailor_project(self, project_dir: Path):
        ...
        for img_path in all_image_paths:
            ...
            if dir_path_str not in dir_map:
                ...
                SubElement(
                    directories_el, "directory", id=str(next_id), path=dir_path_str
                )
                next_id += 1
            ...
            SubElement(
                files_el, "file", dirId=str(dir_id), name=img_path.name, id=str(file_id)
            )
            next_id += 1
            ...
            if PIL_AVAILABLE:
                try:
                    with Image.open(img_path) as im:
                        ...
                        if im.info.get("dpi"):
                            dpi_h, dpi_v = str(int(im.info["dpi"][0])), str(
                                int(im.info["dpi"][1])
                            )
                except Exception as e:
    ...
    def _run_manual_correction(self, workspace: str, project_file_path: Path) -> None:
        ...
        try:
            ...
            logger.error(
                "'flatpak' command not found. Please ensure Flatpak is installed."
            )
            raise
        except Exception as e:
