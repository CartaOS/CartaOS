...
def main():
    ...
    parser.add_argument(
        "--minimal",
        action="store_true",
        help="Install only essential tools (git, python, pipx, poetry).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--ci",
        action="store_true",
        help="Run in CI mode with simplified, non-colored output.",
    )
    ...
    try:
        installer.console.print(
            Panel.fit(
                "[bold blue]🚀 Starting CartaOS Development Environment Setup[/bold blue]"
            )
        )
        installer.run_all()
