...
class ExternalCallManager:
    ...
    def from_env(cls) -> "ExternalCallManager":
        ...
        return cls(
            ...
            circuit_breaker_threshold=int(
                os.getenv("CARTAOS_CIRCUIT_BREAKER_THRESHOLD", "5")
            ),
            circuit_breaker_recovery_timeout=float(
                os.getenv("CARTAOS_CIRCUIT_BREAKER_RECOVERY", "60.0")
            ),
        )
    ...
    async def call_with_retry(
        ...
        # Exhausted retries for non-timeout error: surface as tenacity.RetryError
        # Tests expect RetryError when max retries are exceeded.
        # Pass the last exception object for context; RetryError stores it as last_attempt.
        raise RetryError(
            cast(
                Any,
                last_exception
                or Exception(f"Max retries ({self.max_retries}) exceeded"),
            )
        )
