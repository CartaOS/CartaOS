...
class ContentAwareTriage:
    ...
    def detect_document_type(self, file_path: str) -> DocumentType:
        ...
        if self.config.enable_caching and file_path in self._type_cache:
            ...
            try:
                ...
                if (
                    time.time() - cache_time < self.config.cache_ttl
    ...
    def _detect_by_signature(self, signature: bytes, extension: str) -> DocumentType:
        ...
        # Image signatures
        if (
            signature.startswith(b"\xff\xd8\xff")  # JPEG
            or signature.startswith(b"\x89PNG")  # PNG
            or signature.startswith(b"GIF8")  # GIF
            or signature.startswith(b"BM")  # BMP
    ...
    def _analyze_text_complexity(self, file_path: str) -> float:
        ...
        try:
            ...
            technical_terms = len(
                re.findall(r"\b\w*\d+\w*\b|\b\w*[α-ωΑ-Ω]\w*\b", content)
            )
    ...
    def _extract_image_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        path = Path(file_path)
        return {"format": path.suffix.lower().lstrip("."), "filename": path.name}
    ...
    def make_triage_decision(self, file_path: str) -> TriageDecision:
        ...
        try:
            ...
            logger.info(
                f"Making triage decision for {file_path}: "
            ...
            if (
                ...
                and analysis.size_bytes < 1024 * 1024  # < 1MB
                and analysis.document_type
                in [DocumentType.PDF, DocumentType.IMAGE, DocumentType.TEXT]
            ):
                return TriageDecision(
...
def process_new_file(file_path: str) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Moved {file_path} to {decision.target_stage} "
