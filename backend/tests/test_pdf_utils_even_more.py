...
def test_extract_text_short_pdfplumber_then_fitz_success(
    ...
    short_page.extract_text.return_value = "short text"
    mock_pdf = MagicMock()
    mock_pdf.__enter__.return_value.pages = [short_page]
    mock_pdfplumber_open.return_value = mock_pdf

    # fitz returns non-empty
    fpage = MagicMock()
    fpage.get_text.return_value = "fitz wins"
    mock_doc = MagicMock()
...
@patch("cartaos.utils.pdf_utils.pdfplumber.open")
@patch("cartaos.utils.pdf_utils.fitz.open")
def test_extract_text_fitz_whitespace_stripped_to_empty_string(
    mock_fitz_open, mock_pdfplumber_open, tmp_path: Path
):
    pdf = tmp_path / "w.pdf"
    pdf.write_bytes(b"%PDF-1.4\n")
