...
def test_process_file_endpoint():
    ...
    with patch("pathlib.Path.exists") as mock_exists, patch(
        "cartaos.utils.pdf_utils.extract_text"
    ) as mock_extract:
        mock_exists.return_value = True
        mock_extract.return_value = (
            "This is a long text with more than 500 characters." * 20
        )
...
def test_triage_endpoint():
    ...
    with patch("cartaos.triage.TriageProcessor.process") as mock_triage, patch(
        "pathlib.Path.exists"
    ) as mock_exists:
        mock_exists.return_value = True
        mock_triage.return_value = {"destination": "03_Lab", "reason": "Needs OCR"}

        response = client.post("/api/triage", json={"file_path": "/path/to/file.pdf"})
        assert response.status_code == 200
        data = response.json()
...
def test_ocr_endpoint():
    ...
    with patch("cartaos.ocr.OcrProcessor.process") as mock_ocr, patch(
        "pathlib.Path.exists"
    ) as mock_exists:
        mock_exists.return_value = True
        mock_ocr.return_value = True

        response = client.post("/api/ocr", json={"file_path": "/path/to/file.pdf"})
        assert response.status_code == 200
        data = response.json()
...
def test_summarize_endpoint():
    ...
    with patch("cartaos.utils.ai_utils.generate_summary") as mock_summarize, patch(
        "pathlib.Path.exists"
    ) as mock_exists, patch("cartaos.utils.pdf_utils.extract_text") as mock_extract:
        mock_exists.return_value = True
        mock_extract.return_value = "Sample text content"
        mock_summarize.return_value = "This is a summary of the document."

        response = client.post(
            "/api/summarize", json={"file_path": "/path/to/file.pdf"}
        )
        assert response.status_code == 200
        data = response.json()
...
def test_error_handling():
    ...
    response = client.post(
        "/api/process",
        json={"file_path": "/nonexistent/file.pdf", "operation": "triage"},
    )
