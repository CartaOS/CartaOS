...
def test_full_pipeline_with_mocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    long_pdf = triage_dir / "long.pdf"; long_pdf.write_bytes(b"%PDF-1.4\n")
    short_pdf = triage_dir / "short.pdf"; short_pdf.write_bytes(b"%PDF-1.4\n")
    ...
    monkeypatch.setattr("cartaos.processor.generate_summary", lambda t: "Integration summary")

    # - force summaries to be written into a vault/Summaries under tmp
    vault = tmp_path / "vault"; (vault / "Summaries").mkdir(parents=True, exist_ok=True)
