...
from cartaos.utils.external_calls import (CircuitBreakerError,
                                          ExternalCallManager)
from cartaos.utils.external_calls import TimeoutError as CustomTimeoutError
...
class TestExternalCallManager:
    ...
    async def test_circuit_breaker_half_open_recovery(self):
        ...
        manager = ExternalCallManager(
            circuit_breaker_threshold=2, circuit_breaker_recovery_timeout=0.1
        )
...
class TestAIUtilsIntegration:
    ...
    async def test_generate_content_with_timeout(self):
        ...
        with patch(
            "cartaos.utils.ai_utils.get_external_call_manager"
        ) as mock_get_manager:
            mock_manager = MagicMock()
            mock_get_manager.return_value = mock_manager
            mock_manager.call_with_retry = AsyncMock(
                side_effect=CustomTimeoutError("Timeout")
            )
    ...
    async def test_ocr_retry_on_temporary_failure(self):
        ...
        with patch("cartaos.utils.ocr_utils.ExternalCallManager") as mock_manager:
            ...
            mock_instance.call_with_retry = AsyncMock(
                side_effect=[Exception("Temporary OCR failure"), "OCR success"]
            )
            mock_manager.return_value = mock_instance
...
class TestProgressIndicators:
    ...
    async def test_progress_callback_called(self):
        ...
        manager = ExternalCallManager(
            max_retries=3, base_delay=0.01, progress_callback=progress_callback
        )
