...
from cartaos.install_dev_env.simplified_installer import (
    SimplifiedInstaller,
    DependencyChecker,
    InstallationGuide,
    DependencyStatus
...
class TestDependencyChecker:
    ...
    def test_check_all_required_dependencies(self):
        ...
        with patch('shutil.which', side_effect=lambda x: '/usr/bin/' + x if x != 'unpaper' else None):
...
class TestSimplifiedInstaller:
    ...
    def test_check_system_dependencies_some_missing(self):
        ...
        mock_checker.check_all_dependencies.return_value = {
            'tesseract': DependencyStatus('tesseract', True, '/usr/bin/tesseract'),
            'unpaper': DependencyStatus('unpaper', False, None, 'Install with: sudo apt-get install unpaper')
        }
        installer.dependency_checker = mock_checker
    ...
    def test_run_checks_only_mode(self):
        ...
        mock_checker.check_python_version.return_value = DependencyStatus('python', True, '3.12.0')
        installer.dependency_checker = mock_checker
    ...
    def test_run_full_setup_mode(self):
        ...
        with patch.object(installer, 'check_system_dependencies', return_value=True), \
