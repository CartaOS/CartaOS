...
def setup_dirs(tmp_path):
    inp = tmp_path / "02_Triage"; inp.mkdir()
    out = tmp_path / "05_ReadyForSummary"; out.mkdir()
...
def test_triage_moves_ebook_and_pdf(monkeypatch, setup_dirs):
    ...
    monkeypatch.setattr(TriageProcessor, "_get_file_type", lambda self, p: "pdf" if p.suffix==".pdf" else "ebook")
    monkeypatch.setattr("cartaos.triage.extract_text", lambda p: "x"*600 if "good" in p.name else "")
