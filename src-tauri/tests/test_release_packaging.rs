...
mod release_packaging_tests {
    ...
    fn test_tauri_config_exists() {
        ...
        assert!(config_path.exists(), "tauri.conf.json should exist for packaging");
        
        let config_content = fs::read_to_string(config_path)
            .expect("Should be able to read tauri.conf.json");
        
        // Verify essential packaging fields are present
        assert!(config_content.contains("\"productName\""), "Config should have productName");
        assert!(config_content.contains("\"version\""), "Config should have version");
        assert!(config_content.contains("\"identifier\""), "Config should have identifier");
        assert!(config_content.contains("\"bundle\""), "Config should have bundle configuration");
    }
    ...
    fn test_cargo_toml_has_required_metadata() {
        ...
        assert!(cargo_content.contains("name ="), "Cargo.toml should have name");
        assert!(cargo_content.contains("version ="), "Cargo.toml should have version");
        assert!(cargo_content.contains("edition ="), "Cargo.toml should have edition");
    }
    ...
    fn test_icons_directory_exists() {
        ...
        assert!(icons_path.exists(), "Icons directory should exist for packaging");
        ...
        assert!(found_icons > 0, "Should have at least one icon file for packaging");
    }
    ...
    fn test_build_configuration_for_release() {
        ...
        if config_path.exists() {
            ...
            assert!(
                config_content.contains("\"build\"") || config_content.contains("\"beforeBuildCommand\""),
    ...
    fn test_no_debug_artifacts_in_release() {
        ...
        if src_dir.exists() {
            ...
            assert!(src_dir.is_dir(), "Source directory should exist and be a directory");
        }
    }
    ...
    fn test_version_consistency() {
        ...
        if cargo_path.exists() && tauri_config_path.exists() {
            ...
            assert!(tauri_has_version, "tauri.conf.json should have version field");
        }
    }
}
