...
mod release_build_verification_tests {
    ...
    fn test_release_binary_exists() {
        ...
        assert!(binary_path.exists(), "Release binary should exist after build");
    ...
    fn test_deb_package_created() {
        ...
        if deb_dir.exists() {
            let entries = fs::read_dir(deb_dir)
                .expect("Should be able to read deb directory");
            ...
            for entry in entries.flatten() {
                ...
                if file_name_str.ends_with(".deb") && file_name_str.contains("CartaOS") {
                    ...
                    assert!(metadata.len() > 1_000_000, "Deb package should be substantial size (>1MB)");
                    break;
                }
            }
    ...
    fn test_appimage_created() {
        ...
        if appimage_dir.exists() {
            let entries = fs::read_dir(appimage_dir)
                .expect("Should be able to read appimage directory");
            ...
            for entry in entries.flatten() {
                ...
                if file_name_str.ends_with(".AppImage") && file_name_str.contains("CartaOS") {
                    ...
                    assert!(metadata.len() > 1_000_000, "AppImage should be substantial size (>1MB)");
                    break;
                }
            }
    ...
    fn test_rpm_package_created() {
        ...
        if rpm_dir.exists() {
            let entries = fs::read_dir(rpm_dir)
                .expect("Should be able to read rpm directory");
            ...
            for entry in entries.flatten() {
                ...
                if file_name_str.ends_with(".rpm") && file_name_str.contains("CartaOS") {
                    ...
                    assert!(metadata.len() > 1_000_000, "RPM package should be substantial size (>1MB)");
                    break;
                }
            }
    ...
    fn test_frontend_assets_bundled() {
        ...
        assert!(frontend_build.exists(), "Frontend build directory should exist");
        ...
        assert!(html_content.contains("<html"), "index.html should contain HTML content");
        assert!(html_content.len() > 100, "index.html should have substantial content");
    }
    ...
    fn test_no_debug_symbols_in_release() {
        ...
        if release_binary.exists() {
            let metadata = fs::metadata(release_binary)
                .expect("Should read release binary metadata");
        ...
        if deb_dir.exists() {
            ...
            for entry in entries.flatten() {
                ...
                if file_name_str.ends_with(".deb") {
                    ...
                    assert!(file_name_str.contains("0.1.0"), "Deb filename should contain version");
                    assert!(file_name_str.contains("CartaOS"), "Deb filename should contain product name");
                    break;
                }
            }
        }
